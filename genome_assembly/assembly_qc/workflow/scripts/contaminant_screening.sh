...
assembly_directory="workflow/resources/assemblies"
...
for assembly in $assemblies_list; do
    ...
    mkdir -p -m 775 "${working_directory}/contamination_report/fcs_vec/${assembly_name}/"
    mkdir -p -m 775 "${working_directory}/contamination_report/fcs_contam/${assembly_name}/"
    ...
    if cp "$assembly" "${tools}/fcsadaptor/input_dir"; then
        ...
        --output-dir "${working_directory}/contamination_report/fcs_vec/${assembly_name}" \
        --euk

        # Prepare run for contaminant screening
        python3 "${tools}/fcsgx/fcs.py" screen genome \
        --fasta "${assembly}" \
        --out-dir "${working_directory}/contamination_report/fcs_contam/${assembly_name}" \
        --gx-db "$db" \
        --tax-id "$taxid"
    else
